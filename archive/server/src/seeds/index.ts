...
async function main() {
  ...
  const password = async (value: string) => bcrypt.hash(value, Number(process.env.BCRYPT_SALT_ROUNDS) || 10);
  const upsertUser = (name: string, email: string, plainPassword: string, role: string) =>
    password(plainPassword).then((hashedPassword) => prisma.user.upsert({
      where: { email },
  ...
  if (!initialStockTransaction) await prisma.stockTransaction.create({
    data: {
  ...
  if (!orderStockTransaction) await prisma.stockTransaction.create({
    data: {
  ...
  if (!existingOrder) await prisma.order.create({
    data: {
